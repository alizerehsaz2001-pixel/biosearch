      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="text-center mb-16 space-y-4">
           ...
           <h1 className="text-4xl sm:text-5xl font-extrabold text-slate-900 tracking-tight">
             Welcome to <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-teal-500">BioSearch Architect</span>
           </h1>
           <p className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed">
             Meet your team of specialized AI Samaritans. Each agent is expertly trained to guide you through a specific stage of your biomedical research journey.
           </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-20">
           {MODES.map((mode) => (
             <div key={mode.id} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-xl hover:border-indigo-100 transition-all duration-300 group">
                ...
                <h3 className="font-bold text-slate-900 text-lg mb-2 group-hover:text-indigo-700 transition-colors">{mode.label}</h3>
                <p className="text-slate-500 text-sm leading-relaxed">
                  "I am here to help you <span className="font-medium text-slate-700">{mode.desc.toLowerCase()}</span>."
                </p>
             </div>
           ))}
        </div>
