...
import { BrainCircuit, Copy, Check, TrendingUp, AlertTriangle, Lightbulb, Scale, ArrowRightCircle, Globe, ExternalLink, ShieldAlert, Share2 } from 'lucide-react';
...
const AnalystResultCard: React.FC<AnalystResultCardProps> = ({ result, onContinue }) => {
  ...
  const handleShare = () => {
    const markdownSnippet = `# Critical Research Synthesis: ${result.originalQuery}\n\n${result.content}\n\n---\n*Generated by Biomaterials Research Architect*`;
      ...
      <div className="p-8">
        ...
        <div className="mt-10 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <button 
                    onClick={handleCopy}
                    className="flex items-center justify-center gap-2 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 px-6 py-3 rounded-xl font-bold transition-all text-sm"
                >
                    {copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                    {copied ? 'Synthesis Copied' : 'Copy Text'}
                </button>

                <button 
                    onClick={handleShare}
                    className="flex items-center justify-center gap-2 bg-violet-50 hover:bg-violet-100 text-violet-700 border border-violet-200 px-6 py-3 rounded-xl font-bold transition-all text-sm"
                >
                    {shared ? <Check className="w-4 h-4 text-green-500" /> : <Share2 className="w-4 h-4" />}
                    {shared ? 'Snippet Copied' : 'Share Markdown'}
                </button>
            </div>
